# General properties for accessing a CSV bases service
...
**ID** is a unique ID for this value webservice. It is only used for tying the properties together and not used for other purposes at the moment.

**url** specifies the URL to access the value webservice.

**split/expr** is used to separate the values. If the service is pure CSV, the split-separator is the value of choice. It simply splits the values by the given expression. For CSV this is usually ";" or "," or "\t", even if CSV would only allow ",". If the simple version is not sufficient, some regular expression can be supplied where each braches-group defines a variable. The examples of split and expr above would lead to the same result.
...
**factors** specifies a factor for a value. This ist useful for systems that supply very hardware related values. Many embedded systems have a value range aligned to binary borders like 0..65535 and don't tend to convert them internally. Usually the application provided from device manufacturer does this in advance to displaying the data. Here we need to do it, before posting the value to event bus. The type of the value will not change. If no conversion is needed, null does not affect the value. Factor conversion is only available to Number-based items.

**types** defines the types of the variable. Again, org.openhab.core.library.items.* contains allowd types with "Item" stripped of the classes names. e.g. NumberItem degrades to Number.

**transforms** is one of the most powerfol properties. It contains a look-up procedure to translate values to other values. It needs a reference ID to define the **var** to translate given in the ...vars-property. The **TransId** defines the transformation specified in this properties file. **TransInVar** references the variable that needs a match with refID to make TransOutVar the wanted value.
...
Powador_6.0_20.0_TL3_V202.properties

All settings for accessing the Powador family 6.0 to 20.0 with Software/Firmware Version 2.02 are described here in detail.
